...
def main():
    ...
    parser.add_argument("--before",type=Path,default=ROOT/"Re.KrtC/build/GeneralBaseline/KrtC")
    ...
    report = {"date_utc":datetime.now(timezone.utc).isoformat(),"rounds":args.rounds,"cpu":args.cpu,
              "platform":platform.platform(), "compiler_flags":{"Before":[],"After":args.after_flag},
    ...
    for name,target in jobs:
        ...
        for index in range(args.rounds+1):
            ...
            for version in order:
                ...
                if version == "After": command += args.after_flag
