...
def prepare(args):
    ...
    report = {"prepared_utc": datetime.now(timezone.utc).isoformat(), "platform": platform.platform(),
              ...
              "gcc": checked([gcc,"--version"]).splitlines()[0], "gcc_flags": flags, "krtc_flags": {"Before": [], "O2": ["-O2"], "O3": ["-O3"]},
...
""")
        ...
        for variant,compiler in compilers.items():
            ...
            build([compiler, *(["-"+variant] if variant in ("O2", "O3") else []), krt,"output",binary])
...
def main():
    ...
    parser.add_argument("--baseline",type=Path,default=ROOT/"Re.KrtC/build/BeforeO3/KrtC")
