...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=HERE/"Validation")
    ...
    for level in (2, 3):
        ...
        with tempfile.TemporaryDirectory(prefix="kairote-syntax-level-") as directory:
            ...
            syntax = isolated/"Test/SyntaxAudit"
            syntax.mkdir(parents=True)
            shutil.copytree(ROOT/"Test/SyntaxAudit/cases", syntax/"cases",
                            ignore=shutil.ignore_patterns("*.kro", "*.o"))
            shutil.copy2(ROOT/"Test/SyntaxAudit/run.sh", syntax/"run.sh")
            shutil.copytree(ROOT/"libs", isolated/"libs",
                            ignore=shutil.ignore_patterns("*.kro", "*.o"))
            wrapper = isolated/"KrtC"
            wrapper.write_text(f'#!/bin/sh\nexec {shlex.quote(str(compiler))} -O{level} "$@"\n')
            wrapper.chmod(0o755)
            stem = f"{label}O{level}Syntax"
            command = ["bash", str(syntax/"run.sh")]
            with (output/(stem+".log")).open("w") as log:
                result = subprocess.run(command, cwd=ROOT, env=dict(environment,KRTC=str(wrapper)),
                                        stdout=log, stderr=subprocess.STDOUT, timeout=300)
            shutil.copy2(syntax/"RESULTS.md",output/(stem+".md"))
            results.append({"name":stem, "command":command, "returncode":result.returncode})
            print(stem, "PASS" if result.returncode == 0 else "FAIL", flush=True)
