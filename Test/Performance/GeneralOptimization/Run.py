...
def prepare(args):
    ...
    compilers = {"Before": args.baseline.resolve(), "After": args.compiler.resolve()}
    ...
    report = {"prepared_utc": datetime.now(timezone.utc).isoformat(), "platform": platform.platform(),
              ...
              "gcc": checked([gcc,"--version"]).splitlines()[0], "gcc_flags": flags,
...
""")
        ...
        c_text = re.sub(r"^(u?int(?:32|64)_t \w+\()", r"__attribute__((noinline)) \1", c_text, flags=re.M)
        ...
        for variant,compiler in compilers.items():
            ...
            build([compiler,krt,"output",binary])
...
def main():
    ...
    parser.add_argument("--baseline",type=Path,default=ROOT/"Re.KrtC/build/GeneralBaseline/KrtC")
    ...
    if args.prepare_only:
        print(f"Prepared {len(report['workloads'])} workloads x 3 variants and retained sources/assembly.",flush=True)
        return
    ...
    if not report["correctness"]["After"] or not report["correctness"]["Gcc"]:
